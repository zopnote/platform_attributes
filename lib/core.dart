...
export 'platform/windows.dart';
export 'platform/android.dart';
export 'platform/baremetal.dart';
export 'platform/macos.dart';
export 'platform/linux.dart';
export 'platform/ios.dart';

import 'platform/windows.dart';
import 'platform/android.dart';
import 'platform/baremetal.dart';
import 'platform/macos.dart';
import 'platform/linux.dart';
import 'platform/ios.dart';

/// Returns the first ``Object`` of Type ``T`` that matches
/// or null. Never throws an exception.
...
class Platform<Attributes extends PlatformAttributes> {
  ...
  static Platform current() {
    final String architectureName = io.Platform.version
        .split("\"")[1]
        .split("_")[0];
    ...
    if (io.Platform.isWindows) {
      ...
      final String osVersionString = io.Platform.operatingSystemVersion
          .split("\"")
          .last;
      final RegExpMatch? buildMatch = RegExp(
        r'Build (\d+)',
      ).firstMatch(osVersionString);
      final int buildNumber = buildMatch != null
    ...
    if (io.Platform.isAndroid) {
      ...
      return Platform.android(
        apiLevel: AndroidAPI(
          0,
          Version.parse(io.Platform.operatingSystemVersion),
        ),
        abi: abi,
      );
    }
  ...
  static Platform<MacOSAttributes> macos({
    ...
    attributes: MacOSAttributes(processor: processor, sdkVersion: sdkVersion),
  );

  /// Creates an iOS platform for the specified [version] and [sdkVersion].
  static Platform<iOSAttributes> ios({required Version sdkVersion}) => Platform(
